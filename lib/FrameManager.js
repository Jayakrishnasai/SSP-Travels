...
export default class FrameManager {
  ...
  async loadBundle() {
    const url = this.options.bundleUrl;
    if (!url) return;
    ...
    if (magic !== 0x50534246) throw new Error('Invalid bundle magic');

    const count = dv.getUint32(4, true);
    const indexByteSize = dv.getUint32(8, true);
    const entryCount = indexByteSize / 8;

    this.frameOffsets = new Uint32Array(entryCount);
    this.frameSizes = new Uint32Array(entryCount);

    for (let i = 0; i < entryCount; i++) {
      const off = 12 + i * 8;
      this.frameOffsets[i] = dv.getUint32(off, true);
      this.frameSizes[i] = dv.getUint32(off + 4, true);
    }

    this.buffer = buf;
  }

  getFrameBlobURL(index) {
    const off = this.frameOffsets[index];
    const sz = this.frameSizes[index];
    const slice = this.buffer.slice(off, off + sz);
  ...
  async loadOne(index, highPriority = false) {
    ...
    try {
      ...
      if (this.buffer && img.src.startsWith('blob:')) {
